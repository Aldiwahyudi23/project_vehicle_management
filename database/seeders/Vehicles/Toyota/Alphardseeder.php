...
class AlphardSeeder extends Seeder
{
    public function run(): void
    {
        $dataPath = database_path('seeders/Vehicles/Toyota/data/alphard.json');

        if (!File::exists($dataPath)) {
            $this->command->error("File data Alphard tidak ditemukan: {$dataPath}");
            return;
        }
