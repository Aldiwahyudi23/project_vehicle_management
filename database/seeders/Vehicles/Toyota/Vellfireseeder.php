...
class VellfireSeeder extends Seeder
{
    public function run(): void
    {
        $dataPath = database_path('seeders/Vehicles/Toyota/data/vellfire.json');

        if (!File::exists($dataPath)) {
            $this->command->error("File data Vellfire tidak ditemukan: {$dataPath}");
            return;
        }
